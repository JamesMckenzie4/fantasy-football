...
def get_player_fixtures(n_players=489):
    ...
    with open(
        "../../data/fantasy-premier-league-data/player-fixtures.csv", "w"
    ) as write_file:
        for i in range(1, n_players + 1):
            print("processing player - {}".format(i))
            player_info_url = "https://fantasy.premierleague.com/drf/element-summary/{}".format(
                i
            )
...
def get_player_types():
    ...
    with open(
        "../../data/fantasy-premier-league-data/player-types.csv", "w"
    ) as write_file:
        all_info_url = "https://fantasy.premierleague.com/drf/bootstrap"
        all_info = requests.get(all_info_url)
