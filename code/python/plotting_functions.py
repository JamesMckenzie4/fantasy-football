...
def compare_player_fixture_difficulties(players=['Lukaku', 'Aguero', 'Zaha'], n_matches_per_player=5):
    ...
    df_players = df_players[df_players['web_name'].isin(players)].loc[:,['id', 'web_name']]
    ...
    count_data = next_5_matches.groupby(['web_name', 'difficulty'])['web_name'].count().to_dict()
    ...
    # Make plot 
    data = [go.Bar(x=players, y=value, name=index + 1) for index, value in enumerate(plot_counts)]
    layout = go.Layout(title='Fixture difficulty', barmode='stack', yaxis=dict(title='Number of Games'))
