...
def get_team_name(match):
    ...
    for p_tag in p_tags:
        team_names.append(
            str(p_tag).split('beta-footnote">')[1].split("<")[0].replace(" ", "_")
        )
...
def get_odds_data(odds_table, is_over_under):
    ...
    selection_odds_map = {}
    if is_over_under:
        odds_table_rows = odds_table.find_all(
            "tr", {"class": "diff-row handicap-participant evTabRow bc"}
        )
    else:
...
def main():
    ...
    for match in matches:
        ...
        for market in MARKETS_LIST:
            ...
            file_name = "../../data/odds-data/{}_{}_{}.csv".format(
                team_1, team_2, market
            )
            if file_name in os.listdir("../../data/odds-data/"):
                print(
                    "{} odds data already obtained for {} vs {}".format(
                        team_1, team_2, market
                    )
                )
            with open(file_name, "w") as write_file:
                # Get odds
                href = match.find_all("td", {"class": "betting link-right"})[0].a[
                    "href"
                ]
                ...
                odds_page = requests.get(
                    "{}{}/{}".format(BASE_URL, href, market), headers=headers
                )
                ...
                if not bookmakers:
                    continue
                # Get the odds dict
                selection_to_odds_map = get_odds_data(
                    odds_table, is_over_under=(market == "total-goals-over-under")
                )
